...
def reload_all_plugins() -> Dict[str, Any]:
    ...
    plugin_dirs = [
        ("neuro_fuzzy_multiagent/plugins", "neuro_fuzzy_multiagent.plugins"),
        ("neuro_fuzzy_multiagent/env", "neuro_fuzzy_multiagent.env"),
        ("neuro_fuzzy_multiagent/core/agents", "neuro_fuzzy_multiagent.core.agents"),
        ("neuro_fuzzy_multiagent/core/sensors", "neuro_fuzzy_multiagent.core.sensors"),
        ("neuro_fuzzy_multiagent/core/actuators", "neuro_fuzzy_multiagent.core.actuators"),
    ]
    for dir_path, pkg_base in plugin_dirs:
        for fname in os.listdir(dir_path):
            if (
                not fname.endswith(".py")
                or fname.startswith("__init__")
                or fname.startswith("base_")
            ):
                continue
            mod_name = fname[:-3]
