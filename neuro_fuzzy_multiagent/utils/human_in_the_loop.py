...
def human_in_the_loop_control(step, agent=None, env=None):
    print(
        "\n[Human-in-the-Loop] Step {}: [p]ause, [r]esume, [s]top, [a]djust agent/env, [c]ontinue?".format(
            step
        )
    )
    ...
    if user_input == "p":
        ...
        if agent is not None:
            try:
                new_epsilon = input(
                    "Enter new epsilon for agent (or blank to skip): "
                ).strip()
                if new_epsilon:
        ...
        if env is not None:
            try:
                new_diff = input(
                    "Enter new difficulty for env (or blank to skip): "
                ).strip()
