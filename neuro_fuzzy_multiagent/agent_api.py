...
multimodal_agent = MultiModalDQNAgent(
    input_dims=MULTIMODAL_INPUT_DIMS, action_dim=MULTIMODAL_ACTION_DIM
)
...
def step_env(request: ActionRequest):
    ...
    state["done"] = done
    logger.info(
        f"/step: actions={request.actions} -> obs={obs}, rewards={rewards}, done={done}"
    )
...
def get_state():
    return {
        ...
        "done": state["done"],
        "agent_knowledge": [
            getattr(agent, "online_knowledge", {}) for agent in state["agents"]
        ],
        "law_violations": [
            getattr(agent, "law_violations", 0) for agent in state["agents"]
        ],
...
def observe_audio(file: UploadFile = File(...)):
    ...
    inputs = nlp_tokenizer(
        transcript, return_tensors="pt", truncation=True, padding=True
    )
    with torch.no_grad():
        outputs = nlp_model(**inputs)
        feature = outputs.last_hidden_state[:, 0, :].squeeze().numpy().tolist()
...
def realworld_act(
    target_type: str = Form(...), action: str = Form(...), config: str = Form(...)
):
    """
...
def explain_action(agent_id: int = Form(...), observation: str = Form(...)):
    ...
    if hasattr(agent, "explain_action"):
        try:
            ...
            if (
                hasattr(agent, "input_dims")
                and isinstance(obs, np.ndarray)
                and obs.dtype == object
            ):
        ...
        raise HTTPException(
            status_code=400, detail="Agent does not support explainability"
        )


@app.post("/learn/online", dependencies=[Depends(verify_api_key)])
def learn_online(
    agent_id: int = Form(...), input: str = Form(...), target: str = Form(...)
):
    """
    ...
    if hasattr(agent, "online_update"):
        ...
        raise HTTPException(
            status_code=400, detail="Agent does not support online learning"
        )
...
def set_agent_env_config(cfg: AgentEnvConfig):
    ...
    if not all(t in AGENT_TYPES for t in cfg.agent_types):
        raise HTTPException(
            status_code=400, detail="Unsupported agent_type in agent_types"
        )
    ...
    reset_all()
    logger.info(
        f"/config: Set env_type={cfg.env_type}, agent_types={cfg.agent_types}, n_agents={cfg.n_agents}, n_obstacles={cfg.n_obstacles}"
    )
