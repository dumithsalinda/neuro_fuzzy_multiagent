...
class NeuroFuzzyFusionAgent:
    ...
    """
    def __init__(self, input_dims, hidden_dim, output_dim, fusion_type='concat', fuzzy_config=None, fusion_alpha=0.5, device=None):
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        # Neural fusion network
        self.neural_fusion = FusionNetwork(input_dims, hidden_dim, output_dim, fusion_type=fusion_type).to(self.device)
        # Fuzzy inference system
        self.fuzzy_system = FuzzyInferenceSystem(fuzzy_config) if fuzzy_config is not None else FuzzyInferenceSystem()
    ...
    def act(self, obs_list):
        ...
        fuzzy_out = self.fuzzy_system.evaluate(obs_vector)  # Returns a scalar or vector depending on rules
        # If output is scalar, convert to vector for fusion
        if np.isscalar(fuzzy_out):
            fuzzy_out = np.full(self.output_dim, fuzzy_out)
        # Fuse
        fused_out = self.fusion_alpha * neural_out + (1 - self.fusion_alpha) * np.array(fuzzy_out)
    ...
    def train_step(self, obs_list, target):
        ...
        features = [torch.tensor(x, dtype=torch.float32, device=self.device).unsqueeze(0) for x in obs_list]
        target = torch.tensor(target, dtype=torch.float32, device=self.device).unsqueeze(0)
