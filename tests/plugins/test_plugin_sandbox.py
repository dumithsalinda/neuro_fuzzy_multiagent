...
def test_plugin_sandbox_cpu_limit():
    ...
    assert not result.success
    assert ("cpu" in result.error.lower() or "killed" in result.error.lower() or "resource" in result.error.lower() or "signal" in result.error.lower())

def test_plugin_sandbox_memory_limit():
    sandbox = PluginSandbox()
    result = sandbox.run(plugin_sandbox_memory_limit, timeout=5)
    assert not result.success
    assert ("memory" in result.error.lower() or "killed" in result.error.lower() or "resource" in result.error.lower() or "signal" in result.error.lower())
