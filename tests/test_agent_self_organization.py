import numpy as np
from core.agent import NeuroFuzzyAgent
...
def test_self_organize_changes_rules():
    ...
    fis_config = {
        ...
        'fuzzy_sets_per_input': [[
            ...
            type('FS', (), {'membership': lambda self, x: np.exp(-0.5*((x-0)/0.5)**2), 'params':[0,0.5], 'tune': lambda self, d: None})(),
            type('FS', (), {'membership': lambda self, x: np.exp(-0.5*((x-1)/0.5)**2), 'params':[1,0.5], 'tune': lambda self, d: None})()
        ]]
    }
    ...
    assert before != after or any(
        r1.consequent != r2.consequent for r1, r2 in zip(agent.model.fis.rules, agent.model.fis.rules)
    )
