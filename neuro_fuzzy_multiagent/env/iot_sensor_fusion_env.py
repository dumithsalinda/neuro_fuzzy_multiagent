...
class IoTSensorFusionEnv(BaseEnvironment):
    ...
    def render(self, mode="human"):
        print(
            f"Agents: {self.agent_positions}, Sensors: T={self.temperature}, H={self.humidity}, L={self.light}"
        )
    ...
    def set_external_input(self, agent_idx, value):
        """
        Inject a real-time value for the given agent index. If value is a dict, override temperature/humidity/light.
        """
        if isinstance(value, dict):
            ...
            # fallback: treat as temperature for agent 0
