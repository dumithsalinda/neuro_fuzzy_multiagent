...
def test_meta_agent_switching():
    ...
    class Always0Agent:
        def act(self, obs, state=None): return 0
        def observe(self, r, n, d): pass
        def reset(self): pass
    class Always1Agent:
        def act(self, obs, state=None): return 1
        def observe(self, r, n, d): pass
        def reset(self): pass

    meta = MetaAgent(
        candidate_agents=[(Always0Agent, {}), (Always1Agent, {})],
        window=3
    )
    env = DummyEnv()
    obs = env.reset()
...
def test_meta_agent_reset():
    meta = MetaAgent(candidate_agents=[(TabularQLearningAgent, {'n_states': 2, 'n_actions': 2}), (DQNAgent, {'state_dim': 2, 'action_dim': 2})])
    meta.reset()
