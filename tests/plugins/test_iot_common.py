...
@pytest.mark.skipif('paho.mqtt.client' not in globals(), reason='paho-mqtt not installed')
def test_mqtt_mode():
    # This test is a placeholder; requires local MQTT broker
    def cb(name, value):
        pass
    device = IoTDevice('mqtt_sensor', mode='mqtt', mqtt_config={'broker': 'localhost', 'topic': 'test/topic'}, callback=cb)
...
def test_http_mode(monkeypatch):
    ...
    device = IoTDevice('http_sensor', mode='http', http_config={'url': 'http://dummy'}, interval=0.1, callback=lambda n, v: results.append((n, v)))
    device.start()
    time.sleep(0.25)
    device.stop()
