...
import pandas as pd
from dashboard.visualization import render_agent_positions, render_group_knowledge, render_group_analytics
...
def test_render_group_knowledge_runs():
    class DummyMAS:
        def __init__(self):
            ...
            self.agents = [MagicMock(share_knowledge=lambda: {"a": 1}), MagicMock(share_knowledge=lambda: {"b": 2})]
    mas = DummyMAS()
    with patch("dashboard.visualization.st", st):
