from dashboard.visualization import render_som_grid_with_agents
...
def test_render_som_grid_with_agents_runs():
    class DummySOM:
        ...
        def predict(self, obs):
            return [(0,0), (1,1)]
    mas = MagicMock()
    mas.agents = [MagicMock(last_observation=np.zeros(3), group=0), MagicMock(last_observation=np.zeros(3), group=1)]
    som = DummySOM()
    with patch("dashboard.visualization.st", st):
        render_som_grid_with_agents(mas, som)
