...
def validate_plugin_from_url(
    source_url: str, plugin_type: str, plugin_name: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            logging.error(
                f"Failed to fetch plugin source: {source_url} (status {r.status_code})"
            )
            return report
        code = r.text
    except Exception as e:
    ...
    try:
        ...
        if plugin_type == "environment":
            found = any(
                "BaseEnvironment" in b.__name__
                for n, b in vars(mod).items()
                if isinstance(b, type)
            )
            if not found:
                report["warnings"].append(
                    "No class inheriting from BaseEnvironment found."
                )
        if plugin_type == "agent":
            found = any(
                "BaseAgent" in b.__name__
                for n, b in vars(mod).items()
                if isinstance(b, type)
            )
            if not found:
        ...
        logging.error(
            f"Error during dynamic import or checks for plugin {plugin_name}: {e}"
        )
