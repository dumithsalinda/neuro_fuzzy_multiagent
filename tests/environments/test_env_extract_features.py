...
def test_multiagent_gridworld_extract_features():
    env = EnvironmentFactory.create("multiagent_gridworld", grid_size=4, n_agents=2, n_obstacles=1)
    obs = env.reset()
    features = env.extract_features()
    assert np.allclose(features, obs)
    assert isinstance(features, np.ndarray)
...
def test_adversarial_gridworld_extract_features():
    env = EnvironmentFactory.create("adversarial_gridworld", grid_size=4, n_pursuers=1, n_evaders=1, n_obstacles=0)
    obs = env.reset()
    features = env.extract_features()
...
def test_multiagent_resource_extract_features():
    env = EnvironmentFactory.create("multiagent_resource", grid_size=4, n_agents=2, n_resources=2)
    obs = env.reset()
    features = env.extract_features()
